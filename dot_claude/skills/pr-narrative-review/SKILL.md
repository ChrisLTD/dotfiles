...
- **Attach the diff to blocks worth scrutiny.** For modified blocks where the exact old-vs-new matters (subtle behavioral changes, anything in "Worth scrutinizing"), also set `diff` to the trimmed unified hunk for the shown range in HTML mode — the reader gets a "view diff" toggle on that block, with the after-view still the default. Don't attach diffs to every block; that defeats the narrative.
...
Default to HTML. Only use markdown if the user explicitly asks for it (e.g. "markdown", "just show it in the terminal", "no HTML").
...
```bash
open .scratch/pr-<number>-narrative.html
```
...
The script's header comment documents the full JSON schema. The essentials: top-level `title`, `entry_point`, `shape`, optional `pr_url`, optional `branch` (the head branch name — shown under the title and on the final slide); a `chapters` array where each chapter has `title`, `prose` (a string with paragraphs separated by blank lines, or an array of paragraph strings; supports `inline code`/**bold**/*em* only), optional per-chapter `concerns` (these render as a margin card beside the chapter — prefer attaching concerns to the chapter they belong to rather than one global list), and `blocks` with `path`, `start`, `end`, `status`, `language`, `code`, optional `changed_lines`, `diff`, and `note`. The `appendix` is a list of `{file, change}` pairs. The jump list is generated automatically from the first block of each chapter — don't build it by hand.
