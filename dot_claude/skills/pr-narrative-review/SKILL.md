...
The script's header comment documents the full JSON schema. The essentials: top-level `title`, `entry_point`, `shape`, optional `pr_url`, optional `branch` (the head branch name — shown under the title and on the final slide), optional `ticket_url` + `ticket_label` (set these when the PR references an issue tracker — check the PR body and branch name for a Linear/Jira/GitHub-issue reference; both links render on the final slide); a `chapters` array where each chapter has `title`, `prose` (paragraphs with `inline code`/**bold**/*em* only), optional per-chapter `concerns` (these render as a margin card beside the chapter — prefer attaching concerns to the chapter they belong to rather than one global list), and `blocks` with `path`, `start`, `end`, `status`, `language`, `code`, optional `changed_lines`, `diff`, and `note`. The `appendix` is a list of `{file, change}` pairs. The jump list is generated automatically from the first block of each chapter — don't build it by hand.
